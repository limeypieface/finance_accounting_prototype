...
logger = get_logger("services.subledger.bank")

BANK_DOCUMENT_TYPES = frozenset({
    "DEPOSIT", "WITHDRAWAL", "TRANSFER", "REVERSAL", "ADJUSTMENT",
    "BANK_FEE", "INTEREST",
})


class BankSubledgerService(SubledgerService):
    """
    Bank subledger service.
    ...
    def post(
        ...
        """Post entry to Bank subledger with GL link."""
        errors = self.validate_entry(entry)
        if errors:
            raise ValueError(f"Bank subledger entry validation failed: {errors}")

        if entry.source_document_type not in BANK_DOCUMENT_TYPES:
            raise ValueError(
                f"Invalid Bank source document type: {entry.source_document_type}. "
                f"Allowed: {sorted(BANK_DOCUMENT_TYPES)}"
            )
    ...
    def get_open_items(
        ...
        """Get open bank items for account."""
