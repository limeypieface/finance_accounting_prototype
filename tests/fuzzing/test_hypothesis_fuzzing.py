...
1. Random amounts don't crash the system
2. Balance invariant holds across all valid postings
3. Idempotency holds regardless of input
...
class TestIdempotencyProperty:
    ...
    @settings(max_examples=20, suppress_health_check=[HealthCheck.too_slow, HealthCheck.function_scoped_fixture])
    def test_same_event_id_same_result(
        self,
        amount,
        session,
        post_via_coordinator,
        ...
        """
        Property: Same source_event_id always returns same result.
        """
