...
def cash_service(
    ...
    """Provide CashService for integration testing. workflow_executor required; party_service + test_actor_party for G14."""
    return CashService(
        session=session,
...
class TestAutoReconciliation:
    ...
    def test_auto_reconcile_all_matched(
        ...
        """All lines match — no adjustment posting needed. Service returns TRANSITION_APPLIED (R29: only kernel may assert POSTED)."""
        lines = [
